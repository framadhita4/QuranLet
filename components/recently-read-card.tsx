  ...
  return <>{recentlyRead && <Link href={`/${recentlyRead.surahInfo?.surah_number}?verse=${recentlyRead.ayah}`} className="w-40 p-3 rounded-lg border-2 dark:border-zinc-300 group hover:glow hover:border-sec-color-light dark:hover:border-sec-color-light transition-all duration-150">
    ...
    <div className="rounded-lg w-full py-4 bg-gray-200 dark:bg-sec-color-dark mt-3">
      <div className="m-auto w-fit text-center">
        ...
        <p className="font-semibold text-gray-700 dark:text-gray-500 text-xs group-hover:text-sec-color-light transition-all duration-150">Ayat {recentlyRead.ayah}</p>
      </div>
    </div>
  </Link>
  }</>
