...
type AppProps = { surahInfo: SurahInfo | undefined };

export default function Navbar({ surahInfo }: AppProps) {
    ...
    <div className={`text-gray-700 sticky z-40 transform duration-700 ${(isActive)? "top-0" : "-top-12"}`}>
      ...
      {surahInfo && 
        <div className='overflow-hidden'>
          ...
          <div className='h-[3px] bg-thr-color-light' style={{width: ((window.scrollY + document.documentElement.clientHeight) / (document.body.scrollHeight / 100)) + "vw"}}/>
        </div>
      }
    </div>
  )
