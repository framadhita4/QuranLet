...
export default function SurahCard({ surahInfo }: AppProps) {
  return <>
    <Link href={`/${surahInfo.surah_number}`} className={`${surahInfo.name.toLowerCase()} transition-all duration-100 active:scale-95 group`}>
      <div className="flex justify-between p-3 group-hover:glow dark:border-zinc-300 border-2 rounded-lg transition-all duration-150 dark:group-hover:border-sec-color-light group-hover:border-sec-color-light">
